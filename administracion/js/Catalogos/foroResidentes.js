console.log("foroResidentes.js");
moment.locale('es');


var videosForo = $('#tb_videosForo').DataTable({
    ajax: {
        url: `${BASE_URL}/administracion/controllers/foro.php?accion=getVideosForo`,
        type: "post",
    },
    lengthMenu: [
        [10, 25, 50, 100, 999999],
    ...
    columns: [
        ...
        { 
            data: 'fecha_texto',
            render: function(data, type, row, meta) {
                return `${moment(row.fecha_hora_inicio).format('LL')}`;
                //return `${data}`;
            }
        },
        { 
        ...
        { 
            ...
            render: function(data, type, row, meta) {
                return  `<div class="btn-group mb-3" role="group" aria-label="Default button group">
                    ...
                    <button class="btn btn-outline-secondary px-2 me-2 ver-portada" title="Ver poster" data-img="${row.portada_video}"><i class="fa-solid fa-image"></i></button>   
                </div> `
...
$(document).on('submit', '#addVideoForo', function(e){
    e.preventDefault();
    let url = `${BASE_URL}/administracion/controllers/foro.php?accion=addVideoForo`;
...
$(document).on('click', '.editar', function(e){
    ...
    $.ajax({
        url: `${BASE_URL}/administracion/controllers/foro.php?accion=getVideoForoById`,
        type: 'POST',
        dataType: 'json',
        data: {
            id_video : id_sesion
...
$(document).on('click', '.ver-portada', function(e){
    ...
    $('#div_imagen').html(`<img src="${BASE_URL}imgs/foro_portadas/portadas_videos/${img}" class="img-fluid rounded">`);
